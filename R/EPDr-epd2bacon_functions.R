# #' Reshape C14 data to BACON format
# #' 
...
# #'
# #' @return Data frame with C14 data in BACON format.
# #' 
# #' @export
# #'
# #' @examples
...
# 
# 
# #' Reshape no-C14 data to BACON format
# #' 
...
# #'
# #' @return Data frame with no-C14 data in BACON format. This data frame can be easily combined with C14 data from \code{\link[EPDr:c14_epd2bacon]{c14_epd2bacon}} using \code{rbind}.
# #' 
# #' @export
# #'
# #' @examples
...
# #' #noc14.bacon <- agebasis_epd2bacon(chron@agebasis)
# #' #all.bacon <- rbind(c14.bacon, noc14.bacon)
# agebasis_epd2bacon <- function(agebasis){
#     if(nrow(agebasis) == 0){
#         stop("Table without data. Not performing conversion, useless for BACON.")
#     }
...
# 
# 
# #' Reshape events data to BACON format
# #' 
# #' This function takes event data to fit into a new table that comply with BACON format.
# #'
# #' @param event Data frame with event data for a particular core (entity) in the EPD DDBB.
# #'
# #' @return Data frame with event data in BACON format.
# #' 
# #' @export
# #'
# #' @examples
...
# #' #event.bacon <- events_epd2bacon(event)
...
# 
# 
# #' Reshape depths data to BACON format
# #'
# #' This function takes depths data, as returned by \code{\link[EPDr:getDepths]{getDepths}}, to comply with BACON format.
# #'
...
# #' #depths_epd2bacon(depths.1)
# depths_epd2bacon <- function(depths){
...
# #' @examples
# #' # TBW
# datation_epd2bacon <- function(datation, chronology_number=NA, include_chron_not_in_c14=NA, include_c14_not_in_chron=NA, 
...
#         stop("Datation object for a core without c14 data. Not performing conversion, useless for BACON.")
#     }
#     
...
#     }
#     
#     # Get agebasis for the chronology and convert for bacon
#     agebasis <- chronology@agebasis
#     agebasis <- agebasis[agebasis$chron_ == chronology_number, ]
#     
#     chron <- agebasis_epd2bacon(agebasis)
#     c14 <- c14_epd2bacon(c14)
#     
...
#         cat("C14 data:\n")
#         .printBACON(c14[c14_in_chron,])
#         cat("\n")
#         cat("Chronology data:\n")
#         .printBACON(chron[chron_in_c14,])
#     }
#     if(length(chron_not_in_c14) > 0){
#         cat("\n")
#         cat("Chronology has additional no-C14 data.\n")
#         cat("Chronology data:\n")
#         .printBACON(chron[chron_not_in_c14,])
...
#         cat("C14 data:\n")
#         .printBACON(c14[c14_not_in_chron,])
...
#         cat("C14 data:\n")
#         .printBACON(c14[c14_conf_age,])
#         cat("Chronology data:\n")
#         .printBACON(chron[chron_conf_age,])
...
#         cat("There are age conflicts between c14 data and the chronology.\n")
#         cat("C14 data:\n")
#         .printBACON(c14[c14_conf_depth,])
#         cat("\n")
#         cat("Chronology data:\n")
#         .printBACON(chron[chron_conf_depth,])
...
#         }
#         events.bacon <- events_epd2bacon(events)
#         output <- rbind(output, events.bacon)
#     }
#     
#     
#     # Create directory to save files for BACON
#     if(!dir.exists(paste("BACON/Cores/", e_, sep=""))){
#         dir.create(paste("BACON/Cores/", e_, sep=""), recursive=TRUE)
#     }
#     
#     # Order dataframe by depths and write to the directory
#     output <- output[order(output$depth),]    
#     utils::write.csv(output, file=paste("BACON/Cores/", e_, "/", e_, ".csv", sep=""), na="", row.names=FALSE)
#     
#     # Extract depth columns for pollen counts and create depths.txt files.
#     if(exists("include_depths")){
#         depths.bacon <- depths_epd2bacon(depths)
#         utils::write.table(depths.bacon, file=paste("BACON/Cores/", e_, "/", e_, "_depths.txt", sep=""), col.names=FALSE, na="", row.names=FALSE)
#         utils::write.table(depths, file=paste("BACON/Cores/", e_, "/", e_, "_depths_ID.txt", sep=""), col.names = F, na="", row.names=FALSE, sep=",")
#     }
#     return(output)
# }
# 
