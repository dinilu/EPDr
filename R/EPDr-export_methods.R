...
#' \code{\link[EPDr:.get_c14]{.get_c14}}, to fit into a new table that comply with CLAM
#' or BACON format. 
#'
#' @param format Character string indicating whether to export to "clam" or to "bacon" format.
#' @param x Data frame or \code{\link[EPDr:epd.entity]{epd.entity}} object with C14 data as
#' those extracted from \code{\link[EPDr:.get_c14]{.get_c14}} or
#' \code{\link[EPDr:get_entity]{get_entity}}.
#' @param y Data frame with geochron table, as the one extracted with
#' \code{\link[EPDr:.get_geochron]{.get_geochron}}. If x is an
#' \code{\link[EPDr:epd.entity]{epd.entity}} object y is not used.
#'
...
#' # c14.clam <- export_c14(c14, "clam")
#' # c14.bacon <- export_c14(c14, "bacon")
#' @rdname export_c14-method
...
setMethod("export_c14", signature(format = "character", x = "data.frame",
  ...
  if (nrow(x) == 0){
    warning("Table without c14 data. Returning an empty data.frame.")
    if (format == "clam"){
      output <- data.frame(lab_ID = NA, C14_age = NA, cal_age = NA,
                           error = NA, reservoir = NA, depth = NA,
                           thickness = NA)[-1, ]
    }else{
      output <- data.frame(labID = NA, age = NA, error = NA, depth = NA)[-1, ]
    }
    return(output)
  }
  ...
  output$reservoir <- NA
  if (format == "clam"){
    output <- subset(output, select = c("lab_ID", "C14_age", "cal_age",
                                        "error", "reservoir", "depth",
                                        "thickness"))
  }
  if (format == "bacon"){
    output <- subset(output, select = c("lab_ID", "C14_age", "error", "depth"))
...
#' 
#' This function takes agebasis data, used in previous chronologies, to fit
#' into a new table that comply with CLAM or BACON format.
#'
#' @param x Data frame or \code{\link[EPDr:epd.entity]{epd.entity}} with agebasis data as those
#' in a agebasis table returned by \code{\link[EPDr:.get_agebasis]{.get_agebasis}} or
#' \code{\link[EPDr:get_entity]{get_entity}}.
#' @param format Character string indicating whether to export to "clam" or to "bacon" format.
#'
#' @return Data frame with no-C14 data in CLAM or BACON format. This data frame can be easily
#' combined with C14 data from \code{\link[EPDr:export_c14]{export_c14}} using \code{rbind}.
#' @references \url{http://www.chrono.qub.ac.uk/blaauw/clam.html}
#' @references \url{http://chrono.qub.ac.uk/blaauw/bacon.html}
#' @examples
#' #library(EPDr)
...
#' #noc14.clam <- export_agebasis(chron@agebasis, "clam")
#' #noc14.bacon <- export_agebasis(chron@agebasis, "bacon")
#' #all.clam <- rbind(c14.clam, noc14.clam)
#' #all.bacon <- rbind(c14.bacon, noc14.bacon)
#' @rdname export_agebasis-method
...
#' @rdname export_agebasis-method
#' @aliases export_agebasis-method
setMethod("export_agebasis", signature(format = "character", x = "data.frame"),
  ...
  if (nrow(x) == 0){
    warning("Table without chronology data. Returning an empty data.frame.")
    if (format == "clam"){
      output <- data.frame(lab_ID = NA, C14_age = NA, cal_age = NA,
                           error = NA, reservoir = NA, depth = NA,
                           thickness = NA)[-1, ]
  ...
  output <- data.frame(lab_ID = paste("E", x$e_, "_CH", x$chron_, "_S",
                                      x$sample_, sep = ""), C14_age = x$age,
                       error = x$ageup - x$age, depth = x$depthcm,
...
#'
#' @param format Character string indicating whether to export to "clam" or to "bacon" format.
#' @param x Data frame or \code{\link[EPDr:epd.entity]{epd.entity}} object with event data as
#' those extracted from \code{\link[EPDr:.get_synevent]{.get_synevent}} or \code{\link[EPDr:get_entity]{get_entity}} 
#' @param y Data frame with event data as those extracted from \code{\link[EPDr:.get_event]{.get_event}}.
#'
...
#' #event.clam <- export_events(event, format = "clam")
#' #event.bacon <- export_events(event, format = "bacon")
#' @rdname export_events-method
...
#' @rdname export_events-method
#' @aliases export_events-method
setMethod("export_events", signature(format = "character", x = "data.frame",
                                    y = "data.frame"), function(format, x, y){
  ...
  if (nrow(z) == 0){
    warning("Table without dated events. Returning an empty data.frame.")
    if (format == "clam"){
      output <- data.frame(lab_ID = NA, C14_age = NA, cal_age = NA, error = NA,
                           reservoir = NA, depth = NA, thickness = NA)[-1, ]
    }else{
      output <- data.frame(labID = NA, age = NA, error = NA, depth = NA)[-1, ]
    }
    return(output)
  }
  output <- data.frame(lab_ID = paste("E", z$e_, "_EV", z$event_, sep = ""),
                       C14_age = z$agebp, error = z$ageuncertup,
                       depth = z$depthcm, thickness = z$thickness)
  output$error[which(is.na(output$error) | output$error == 0)] <- 1
  output$cal_age <- NA
  output$reservoir <- NA
  if (format == "clam"){
...
#' @rdname export_events-method
#' @aliases export_events-method
setMethod("export_events", signature(format = "character", x = "epd.entity",
                                    y = "missing"), function(format, x, y){
...
#'
#' This function takes depths data, as returned by \code{\link[EPDr:.get_psamples]{.get_psamples}},
#' to comply with CLAM or BACON format.
#'
#' @param x Data frame with at least a column called depthcm or \code{\link[EPDr:epd.entity]{epd.entity}} object to extract this information from.
#'
...
#' @param format Character string indicating whether to export to "clam" or to "bacon" format.
#' @param chronology Number indicating the chronology from which to extract the agebasis
...
#' @param incl_chron_not_in_c14 Logical value indicating whether the function should include
...
#' @param incl_c14_not_in_chron Logical value indicating whether the function should include 
...
#' @param use_c14_conf_age Logical value indicating whether the function should use data
...
#' @param use_c14_conf_depth Logical value indicating whether the function should use data
...
#' @param include_depths Logical value indicating whether depths of pollen samples should
...
#' @param incl_envents Logical value indicating whether events information should be
...
setMethod("export_entity", signature(format = "character", x = "epd.entity"),
  ...
  # incl_envents <- NULL
            ...
            .print_data <- function(data, format){
              if (format == "clam"){
                cat(c("lab_ID", "C14_age", "cal_age", "error", "reserv.",
                      "depth", "thickn.\n"), sep = "\t")
              }
              if (format == "bacon"){
            ...
            if (is.null(chronology)){
              ...
              if (!chronology %in% 1:x@numberofchron){
                stop("The chronology does not exist.")
              }
            }
            ...
            if (length(chron_not_in_c14) > 0){
              ...
              while (!.is_true_false(incl_chron_not_in_c14)){
                incl_chron_not_in_c14 <- as.logical(readline(paste("Incorpo",
                                                                   "rate ",
                                                                   "these ",
                                                                   "data to ",
                                                                   "the chron",
